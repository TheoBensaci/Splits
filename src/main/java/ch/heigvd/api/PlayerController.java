...
public class PlayerController {
  ...
  public static userData checkBodyToken(Context ctx, IToken tokenData) {
    ...
    if (pl == null) {
      ...
      data = new userData(pl.name, tokenData.getToken());
    }
    return data;
  }
  ...
  public void createPlayer(Context ctx) {
    PlayerInfo playerInfo =
        ctx.bodyValidator(PlayerInfo.class)
            .check(
                obj -> {
  ...
  public void changePlayerName(Context ctx) {
    ChangeNameData cn =
        ctx.bodyValidator(ChangeNameData.class)
            .check(
                obj -> {
                  return obj.name != null && !obj.name.isEmpty();
                },
    ...
    PlayerBase.getPlayerByToken(cn.token).name = cn.name;
